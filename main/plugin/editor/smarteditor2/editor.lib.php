<?php
if (!defined('_DONG9_')) exit; // 개별 페이지 접근 불가


function editor_html($id, $content, $is_dhtml_editor=true, $use_picture = true, $use_mobile = false)
{;
    ...
    $editor_url = '/main/'.D9_PLUGIN_DIR.'/editor/smarteditor2';//.$config['cf_editor'];
...
function chk_editor_js($id, $is_dhtml_editor=true, $value="")
{
    if ($is_dhtml_editor) {
        return "if (!{$id}_editor_data || jQuery.inArray({$id}_editor_data.toLowerCase(), ['&nbsp;','<p>&nbsp;</p>','<p><br></p>','<p></p>','<br>']) != -1) { alert(\"check ".$value.".\"); oEditors.getById['{$id}'].exec('FOCUS'); return false; }\n";
    } else {
        return "if (!{$id}_editor.value) { alert(\"check ".$value.".\"); {$id}_editor.focus(); return false; }\n";
    }
}
