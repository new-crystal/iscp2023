        ...
        _done : function(e, data){
            ...
            */
			...
			$.each(data.files, function (index, f) {
				...
				form_data.append('editor_upload', f);
				...
				if (f.name.match(special_ch)) {
					alert("파일명에 특수문자가 들어있습니다.");
					node.parent().remove();
					return false;
				} else if (!f.type.match("jpg") && !f.type.match("jpeg") && !f.type.match("png") && !f.type.match("pdf") && !f.type.match("xlsx") && !f.type.match("haansoftdocx") && !f.type.match('hwp') && !f.type.match('haansofthwt')) {
					alert("다음 확장자 외 파일은 첨부할 수 없습니다.\n(.jpg, .jpeg, .png, .pdf, .xlsx, .hwp, .hwpx, .hwt, .hwtx)");
					node.parent().remove();
					return false;
				} else if (f.size >= 104857600) {				// 100MB 기준 => ※ 만약 파일을 다중으로 선택할 시, 100MB 사진만 올라오지 않고 나머지 사진들은 올라오게끔 처리함.
					alert("파일 크기는 100MB 미만으로 등록 해주셔야 합니다.");
					node.parent().remove();
					return false;
				} else {
				...
				$.ajax({
					url : '../../../../../ajax/ajax_editor_upload.php',
					type : 'POST',
					data : form_data,
					dataType : 'JSON',
					processData: false,
