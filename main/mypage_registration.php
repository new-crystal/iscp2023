					...
					<?php
						foreach($registration_list as $list) {
							...
							if($rr_during_yn == 'Y') {
								if($before_price == 250) {
									$before_price = 300;
								} else if($before_price == 100) {
									$before_price = 150;
								} else if($before_price == 80000) {
									$before_price = 100000;
								} else if($before_price == 100000 && $ksola_member_status==0) {
									$before_price = 120000;
								} else if($before_price == 50000) {
									$before_price = 60000;
								}
							}
...
<script>
	...
	function registration_btn(idx) {
		//$(".receipt_pop").show();
		var url = "https://iscp2023.org/main/pre_registration_confirm.php?key=<?=explode('@', $_SESSION['USER']['email'])[0]?>&idx="+idx;
