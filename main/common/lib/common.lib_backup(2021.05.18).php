...
function upload_image($file_obj, $upload_path, $file_directory_idx, $thumb_yn='N', $wish_width=0, $wish_height=0){
	// -------------------------------------------- Date ----------------------------------------------- //
	...
	// ------------------------------------------- Directory -------------------------------------------- //
	// 0:admin = 관리자, 1:directory = 디렉토리
	$file_derectory_list = ["admin", "user", "editor"];
	$file_derectory = $file_derectory_list[$file_directory_idx];

	$absolute_path = D9_IMG_PATH;
	$upload_path = "../../..".$absolute_path;
	
	$upload_path .= "/".$file_directory."/".$year_dir."/".$month_dir."/".$day_dir;
	$upload_db_path = $absolute_path."/".$file_directory."/".$year_dir."/".$month_dir."/".$day_dir;

	if (isset($file_directory_idx) && !empty($file_directory_idx) && ($file_directory_idx == 2)) {
	...
	move_uploaded_file($file_tmp, $file_path."/".$file_save_name);
